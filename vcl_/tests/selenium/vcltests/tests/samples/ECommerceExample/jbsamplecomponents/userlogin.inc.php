<?php
        ...
        class UserLogin extends GraphicControl
        {
            ...
            function LoginUser( $username, $password )
            {
                ...
                $query->LimitStart = '-1';
                $query->LimitCount = '-1';
                $query->SQL = 'SELECT * FROM ' . $this->UserTable . " WHERE Username = '" . $username . "' AND Password = '" . $password . "'";
                $query->open();
                ...
                $this->Database->execute( 'UPDATE ' . $this->UserTable . " SET `LoginID` = '" . $loginid . "' WHERE Username = '" . $username . "' AND Password = '" . $password . "'" );
            ...
            function LogoutUser()
            {
                ...
                $this->Database->execute( 'UPDATE ' . $this->UserTable . " SET `LoginID` = '' WHERE `LoginID` = '" . $loginid . "'" );
            ...
            protected function checkIfLoggedIn()
            {
                ...
                $query->LimitStart = -1;
                $query->LimitCount = -1;
                $query->SQL = 'SELECT Username FROM ' . $this->UserTable . " WHERE `LoginID` = '" . $loginid . "'";
                $query->open();
