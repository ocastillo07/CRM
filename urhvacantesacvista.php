...
//Class definition
class urhvacantesacvista extends Page
{
       public $sqlgrid = null;
       public $dsgrid = null;
   ...
   function urhvacantesacvistaJSLoad($sender, $params)
   {
?>
...
<?php
   ...
   function urhvacantesacvistaCreate($sender, $params)
   {
      if(Derechos('ACCRHVACAC') == false)
      {
         echo '<script language="javascript" type="text/javascript">
               alert("Usted no tiene derechos para Accesar Vacantes Aumento de Colaboradores");
   ...
   function urhvacantesacvistaShow($sender, $params)
   {
   ...
   {
      if(Derechos('EXPRHVACAC') == false)
      {
         echo '<script language="javascript" type="text/javascript">
              alert("No tienes Derechos para Exportar Vacantes Aumento de Colaboradores");
              </script>';
      }
      else
      {
         exportar($this->sqlgrid->sql, 'Vacantes Aumento de Colaboradores');
      }
   }
...
?>
         if(!confirm('Desea Eliminar la Accion Seleccionada?'))
            return(false);
         document.location.href("urhvacantesac.php?idsolicitud="+document.getElementById("hfidsolicitud").value+"&elim=1");
<?php
   }
   ...
   function filtro()
   {
      ...
      switch($this->cbofiltro->ItemIndex)
      {
         ...
         case 6:
            //fecharegreso
            $cond = ' v.cantidad = "' . $this->edbuscar->Text . '"';
            break;
      }
      ...
      $sql = 'SELECT v.idsolicitud, v.idestatus, v.idnota, v.fechacreacion,
              v.cantidad, v.observaciones, pl.nombre as plaza, p.nombre as puesto,
              a.nombre as area, e.nombre as estatus, v.idoriginador
              from rhvacantesac v left join rhestatus_vac e on e.idestatus=v.idestatus
   ...
   {
?>
      document.location.href("urhvacantesac.php?idsolicitud="+document.getElementById("hfidsolicitud").value);
<?php
   }
...
?>
      ...
      document.urhvacantesacvista.hfidsolicitud.value = model.getValue(0, row);
<?php
   }
}
...
global $urhvacantesacvista;

//Creates the form
$urhvacantesacvista=new urhvacantesacvista($application);

//Read from resource file
$urhvacantesacvista->loadResource(__FILE__);

//Shows the form
$urhvacantesacvista->show();
